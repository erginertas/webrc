...
const PrescriptionOrderSummery = ({data, trackOrderData}) => {
    ...
    return (
        <CustomStackFullWidth sx={{padding: "7px 0px 20px 0px"}} spacing={2}>
            <Typography variant="h6">{t("Prescription")}</Typography>
            <Stack height="270px" justifyContent="space-between">
                <Stack direction="row" gap="10px" flexWrap="wrap">
                    {
                        data?.order_attachment?.map((attachment, index) => {
                            return (
                                <Stack key={index} onClick={() => handleImageOnClick(attachment)}
                                       sx={{cursor: "pointer",}}>
            ...
            <CustomModal openModal={openModal} handleClose={handleModalClose}>
                <Stack position='relative'>
                    ...
                    <CustomImageContainer src={`${configData?.base_urls?.order_attachment_url}/${modalImage}`}
                                          width="600px" smWidth="300px" objectfit="contain"/>
                </Stack>
            </CustomModal>
        </CustomStackFullWidth>
    );
