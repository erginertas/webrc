...
const Item = ({image, title, link, t}) => {
    ...
    return <ItemWrapper onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onClick={handleClick}>
                ...
                <Paper sx={{marginTop: '6px'}}>
                    {
                        ...
                        <Item image={playstore} title={t('Google Play')} link={urls?.playStoreUrl} t={t}/>
                    }
                    {
                        (urls?.playStoreStatus === '1' && urls?.appStoreStatus === '1') ? <CustomDivider/> : null
                    }
                    {
                        urls?.appStoreStatus === '1' &&
                        <Item image={appleLogo} title={t('App Store')} link={urls?.appStoreUrl} t={t}/>
                    }
                </Paper>
            </Popper>
        </ClickAwayListener>
